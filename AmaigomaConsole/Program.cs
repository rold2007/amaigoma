...
namespace AmaigomaConsole
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            sourceImages.Add(Image.Load<L8>(imageMainPath + @"Images\FS18800114.2.11-a2-427w-c32\a\a.png"));

            foreach (string filename in Directory.EnumerateFiles(imageMainPath + @"Images\FS18800114.2.11-a2-427w-c32\Noise\"))
            {
                sourceImages.Add(Image.Load<L8>(filename));
            }

            foreach (string filename in Directory.EnumerateFiles(imageMainPath + @"Images\FS18800114.2.11-a2-427w-c32\NotA\"))
            {
                sourceImages.Add(Image.Load<L8>(filename));
            }
